...
function toggleBox() {
      ...
      const optionsBox = document.getElementById('optionsBox');
      const circle = document.querySelector('.profileFrame');

      // If click is outside both the circle and options box
          if (!circle.contains(event.target) && !optionsBox.contains(event.target)) {
          optionsBox.classList.remove('open');
          }
      });

      // Prevent event bubbling to document when clicking on the circle or the box itself
      document.querySelector('.profileFrame').addEventListener('click', function(event) {
          event.stopPropagation();
      });

      document.getElementById('optionsBox').addEventListener('click', function(event) {
          event.stopPropagation();
      });